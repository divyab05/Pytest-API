...
class Get_Supported_Version:
    ...
    def send_get_devicehub_supported_versions(self, token_type, access_token, invalid_resource_path):

        if type(invalid_resource_path) == str:
            base_uri = str(
                self.app_config.env_cfg['base_uri_devicehub'] + device_hub_resource_path.getDHSupportedVersion) + \
        ...
        self.custom_logger.info("Send Get request to check devicehub by Supported version"
        ...
        self.custom_logger.info("Response payload for Get devicehub by Supported version"
