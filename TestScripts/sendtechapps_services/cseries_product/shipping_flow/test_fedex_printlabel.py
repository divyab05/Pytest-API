...
def resource(app_config,context):
    shippinglabel = {
        ...
        'shippinglabel': ShippingLabel(app_config,context['cseries_okta_token'],"fedex"),
...
class TestFedExPrintLabel(common_utils):
    ...
    def getCarrierProfileIdFromCustomerContextAPI(self, resource):
        ...
        if "fedex" not in carrierProfileArray:
            pytest.skip("Skip FedEx Cases due to not having carrier added in this account")
    ...
    @pytest.mark.parametrize("test_data",
                             common_utils.read_excel_data_store("sendtechapps_services",
                                                                ...
                                                                "FedExDomLabelPrint"))
    def test_fedex_print_request_domestic(self,resource,test_data,
                                                 custom_logger,getCarrierProfileIdFromCustomerContextAPI):
        if self.carrierProfileId=="" :
            pytest.skip("carrier not exist")
    ...
    @pytest.mark.parametrize("test_data",
                             common_utils.read_excel_data_store("sendtechapps_services",
                                                                "PrintLabelTestData_US.xlsx",
                                                                "FedExIntLabelPrint"))
    def test_fedex_print_request_int(self, resource, test_data,
                                custom_logger,getCarrierProfileIdFromCustomerContextAPI):
