...
class Test_get_order_api(common_utils):
    ...
    def test_get_order_api_shopify_sso_with_wh_flg(self, resource, get_integrator_id, generate_access_token, context):
        ...
        #                                             self.read_json_file('Orders_wh_true.json',
        #                                                                 'ecommerce_services'))

        # resource.store_order_info(resp['response_body'], context)

        # assert resource.validate_date_format_in_orders(resp['response_body']['orders'][0]) is True
